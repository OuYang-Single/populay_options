...
import static com.github.mikephil.charting.utils.DataTimeUtil.getDate;
...
public class KLineDataManage {
    ...
    public void parseKlineData(List<KLineDataModel> mKLineDataModelLsit, String assetId,boolean landscape,String type) {
        ...
        if (mKLineDataModelLsit != null) {
            kDatas.clear();
            lineDataMA.clear();
            kDatas.addAll(mKLineDataModelLsit);
                xVal.clear();
            ...
            double ma20 = 0;
                for (int i = 0; i < kDatas.size(); i++) {
                    if (i!=0){
                        preClosePrice = mKLineDataModelLsit.get(i-1).getClose();
                    }
                    mKLineDataModelLsit.get(i).m_szInstrumentID=assetId;
                    xVal.add(DataTimeUtil.secToDate(getDate(getKLineDatas().get(i).getDateMills()),this.type));
                    KLineDataModel point = mKLineDataModelLsit.get(i);
                    final double closePrice = point.getClose();
                    ma5 += closePrice;
                    ma10 += closePrice;
                    ma20 += closePrice;
                    if (i >= 5) {
                        ma5 -= mKLineDataModelLsit.get(i - 5).getClose();
                        point.setMa5( ma5 / 5f);
                    } else {
                        point.setMa5(ma5 / (i + 1f));
                    }
                    if (i >= 10) {
                        ma10 -= mKLineDataModelLsit.get(i - 10).getClose();
                        point.setMa10 (ma10 / 10f );
                    } else {
                        point.setMa10 ( ma10 / (i + 1f));
                    }
                    if (i >= 20) {
                        ma20 -= mKLineDataModelLsit.get(i - 20).getClose();
                        point.setMa20( ma20 / 20f);
                    } else {
                        point.setMa20(  ma20 / (i + 1f));
                    }
    ...
    private CandleDataSet setACandle(ArrayList<CandleEntry> candleEntries) {
        ...
        candleDataSet.setValueTextSize(0.00001f);
    ...
    private BarDataSet setABar(ArrayList<BarEntry> barEntries, String label) {
        ...
        barDataSet.setValueTextSize(0.00001f);
